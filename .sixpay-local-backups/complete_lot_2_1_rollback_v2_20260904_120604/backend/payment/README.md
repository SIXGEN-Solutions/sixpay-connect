...
## Post-confirmation authorization gate

After a bank-owned confirmation challenge reaches `VERIFIED`, Payment enters
`AUTHORIZATION_CHECKING`.

This state is a SIXPAY-local business gate. It is not a second TRESOR PAY
authorization-token verification and it is not a Core Banking funds check.

The gate records one durable internal decision covering exactly:

- partner authorization;
- subscription authorization;
- client-application authorization;
- claim-type authorization;
- requested execution-date validity;
- request-data consistency.

An approved local decision progresses Payment to `FUNDS_CONTROL_PENDING` and
emits `PaymentFundsControlRequested`. A rejected local decision terminates the
Payment as `REJECTED` with a safe Payment failure at the `AUTHORIZATION` stage.

The gate result contains no bearer token, JWT, OTP or provider payload. The
actual adapters/services that resolve the authoritative SIXPAY business
referentials producing these six checks are outside LOT 2.1 and must not be
invented inside Payment.
