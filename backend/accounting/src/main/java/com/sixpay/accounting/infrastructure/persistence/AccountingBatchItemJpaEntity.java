...
public class AccountingBatchItemJpaEntity {
    ...
    @Column(name="financial_snapshot_version", updatable=false, length=32) private String financialSnapshotVersion;
    @Column(name="financial_snapshot_finalized_at", updatable=false) private Instant financialSnapshotFinalizedAt;
    @Column(name="debtor_account_reference", updatable=false, length=256) private String debtorAccountReference;
    @Column(name="creditor_account_reference", updatable=false, length=256) private String creditorAccountReference;

    @OneToMany(mappedBy="item", cascade=CascadeType.ALL, orphanRemoval=true)
    private List<AccountingBatchItemEntryJpaEntity> entries = new ArrayList<>();
