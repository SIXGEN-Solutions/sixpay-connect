...
public record AmplitudePaymentBankingProperties(
        @NotNull URI baseUrl,
        @NotBlank String verificationPath,
        @NotBlank String fundsPath,
    ...
    public static final String PREFIX =
            "sixpay.payment.banking.amplitude";

    public AmplitudePaymentBankingProperties {
        validateBaseUrl(baseUrl);
        verificationPath = path(
                verificationPath,
                "verificationPath"
        );
        fundsPath = path(fundsPath, "fundsPath");
    ...
    public record Security(
        ...
        public Security {
            ...
            sslBundle = text(sslBundle, "sslBundle");
        }
    }
    ...
    public record Contract(
            ...
            @NotNull Set<@NotBlank String> rejectionCodes
    ) {
        public Contract {
            version = text(version, "version");
            successCodes = Set.copyOf(successCodes);
            rejectionCodes = Set.copyOf(rejectionCodes);
            if (successCodes.isEmpty()) {
                throw new IllegalArgumentException(
                        "At least one success code is required"
                );
            }
        }
    }
