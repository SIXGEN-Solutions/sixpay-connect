...
import com.sixpay.payment.infrastructure.outbox.serialization
        .PaymentOutboxSerializationException;
import com.sixpay.payment.infrastructure.outbox.serialization
        .UnknownPaymentOutboxEventTypeException;
...
class PaymentObservedCustomerOutboxDispatcherTest {
    ...
    void appliedReplayAndStaleAreAcknowledgedOnlyAfterCustomer() {
        for (ObservedCustomerProjectionResult.Disposition disposition
            ...
            var result = dispatch(fixture).getFirst();
            ...
            ordered.verify(fixture.handler)
                    .handle(fixture.event);

            ordered.verify(fixture.completion)
                    .markPublished(
    ...
    void temporaryInfrastructureFailureSchedulesRetry() {
        ...
        verify(fixture.completion, never())
                .markPublished(
                        fixture.claim.eventId(),
                        fixture.claim.claimedBy(),
                        NOW
    ...
    @Test
    void exponentialBackoffUsesAttemptAndMaximumBound() {
        Fixture thirdAttempt = fixture(3);

        doThrow(
                new DataAccessResourceFailureException(
                        "temporary"
                )
        ).when(thirdAttempt.handler)
                .handle(thirdAttempt.event);

        var thirdResult = dispatch(
                thirdAttempt,
                10,
                Duration.ofSeconds(2),
                Duration.ofMinutes(1)
        ...
        assertEquals(
                NOW.plusSeconds(8),
                thirdResult.nextAttemptAt()
        );

        Fixture boundedAttempt = fixture(8);

        doThrow(
                new DataAccessResourceFailureException(
                        "temporary"
                )
        ).when(boundedAttempt.handler)
                .handle(boundedAttempt.event);

        var boundedResult = dispatch(
                boundedAttempt,
                10,
                Duration.ofSeconds(2),
        ...
        assertEquals(
                NOW.plusSeconds(30),
                boundedResult.nextAttemptAt()
        );
    }

    @Test
    void exhaustedRetryableFailureBecomesDeadLetter() {
    ...
    @Test
    void unknownContractIsDeadLetteredWithoutCustomerCall() {
        Fixture fixture = fixture(1);

        when(fixture.deserializer.deserialize(
                fixture.claim.payload()
        )).thenThrow(
    ...
    @Test
    void malformedPayloadIsDeadLetteredWithoutCustomerCall() {
        Fixture fixture = fixture(1);
        ...
        when(fixture.deserializer.deserialize(
                ...
                new PaymentOutboxSerializationException(
                        "Cannot decode Payment outbox payload"
                )
        );
    ...
    @Test
    void identityMismatchIsDeadLetteredBeforeCustomerAndAck() {
        Fixture fixture = fixture(1);

        ObservedCustomerProjectionEvent conflicting =
                event(
                        UUID.fromString(
    ...
    private static List<PaymentObservedCustomerOutboxResult>
            dispatch(Fixture fixture) {
    ...
    private static List<PaymentObservedCustomerOutboxResult>
        ...
        return fixture.dispatcher.dispatchAvailable(
                NOW,
                "worker-a",
                10,
    ...
    private static Fixture fixture(int attempt) {
        ...
        var dispatcher =
                new PaymentObservedCustomerOutboxDispatcher(
                        claimService,
                        deserializer,
                        handler,
    ...
    private static ObservedCustomerProjectionEvent event(
        ...
        Instant createdAt =
                Instant.parse("2026-08-04T17:55:00Z");
