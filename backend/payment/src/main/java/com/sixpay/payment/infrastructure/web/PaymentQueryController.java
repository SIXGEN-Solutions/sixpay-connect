...
public class PaymentQueryController {
    ...
    public PaymentQueryController(PaymentProjectionQueryUseCase queryUseCase, PaymentQueryRestMapper mapper) {
        this.queryUseCase = queryUseCase;
        this.mapper = mapper;
    }

    @GetMapping
    public ResponseEntity<PaymentQueryResponses.PaymentSearchPageResponse> searchPayments(
            @RequestHeader("X-Correlation-ID") UUID correlationId,
            @RequestParam(required=false) @Size(max=2048) String cursor,
            @RequestParam(defaultValue="50") @Min(1) @Max(200) int size,
            @RequestParam(required=false) @Size(max=64) String paymentReference,
            @RequestParam(required=false) @Size(max=100) String tresorPayRequestId,
            @RequestParam(required=false) UUID observedCustomerId,
            @RequestParam(required=false) @Size(max=32) String financialInstitutionCode,
            @RequestParam(required=false) String status,
            @RequestParam(required=false) @Size(max=64) String reasonCode,
            @RequestParam(required=false) @DateTimeFormat(iso=DateTimeFormat.ISO.DATE_TIME) Instant createdFrom,
            @RequestParam(required=false) @DateTimeFormat(iso=DateTimeFormat.ISO.DATE_TIME) Instant createdTo,
            @RequestParam(required=false) BigDecimal amountMin,
            @RequestParam(required=false) BigDecimal amountMax,
            @RequestParam(required=false) @Pattern(regexp="^[A-Z]{3}$") String currency,
            @RequestParam(defaultValue="CREATED_AT_DESC") PaymentSearchSort sort
    ) {
        var page = queryUseCase.search(new SearchPaymentProjectionsQuery(
                cursor, size, paymentReference, tresorPayRequestId,
                observedCustomerId, financialInstitutionCode, status,
                reasonCode, createdFrom, createdTo, amountMin, amountMax,
                currency, sort
        ));
    ...
    public ResponseEntity<PaymentQueryResponses.PaymentDetailResponse> getPayment(
            @RequestHeader("X-Correlation-ID") UUID correlationId,
            @PathVariable UUID paymentId
    ) {
        var detail = queryUseCase.findById(paymentId)
                .orElseThrow(() -> new PaymentProjectionNotFoundException(paymentId));
        return ResponseEntity.ok()
                .header("X-Correlation-ID", correlationId.toString())
