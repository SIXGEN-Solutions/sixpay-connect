...
class PaymentInitiationOrchestrationServiceTest {
    ...
    @Test void continuesSynchronouslyUntilActiveOtpChallenge() {
        ...
        InitiatePaymentCommand command=command(); PaymentId id=new PaymentId(UUID.randomUUID()); PublicPaymentReference ref=PublicPaymentReference.of("PAY-1234567890ABCDEFGHJKMNPQRS"); NewPaymentIntent intent=Mockito.mock(NewPaymentIntent.class);
        when(idem.execute(any(),any())).thenAnswer(i->{ @SuppressWarnings("unchecked") Function<String,PaymentInitiationResult> f=i.getArgument(1); return f.apply("a".repeat(64)); });
    ...
    private static InitiatePaymentCommand command(){ return new InitiatePaymentCommand(PaymentSource.of("PARTNER"),ExternalSubscriptionReference.of("PARTNER:TP_APP_001"),CanonicalPartnerIdentity.from("11111111-2222-3333-4444-555555555555"),"TP_APP_001","AVI-2025-00045678",new BigDecimal("600000"),"XAF","10005-00001-12345678901-12","Société ABC SARL",ClaimType.AVI,"100200300",NOW,List.of(new PaymentBeneficiaryCommand("10005-00001-TRESDGI-97",new BigDecimal("600000"))),"https://partner.cm/callback","IDEMPOTENCY-00000001",CorrelationId.of("11111111-1111-1111-1111-111111111111")); }
}
