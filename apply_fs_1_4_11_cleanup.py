...
POLICY_SCRIPT = ROOT / "frontend/scripts/verify-runtime-datasource-policy.mjs"

POLICY_SOURCE = r"""import fs from 'node:fs';
...
def main() -> None:
    if not (ROOT / "ENGINEERING_CONTEXT.md").is_file():
        fail(
            "Run this script from the root of the sixpay-connect repository."
        )

    if not PACKAGE_JSON.is_file():
        fail(f"Missing {PACKAGE_JSON}")

    POLICY_SCRIPT.parent.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    scripts[
        ...
        "node scripts/"
        "verify-runtime-datasource-policy.mjs"
    )

    current_integration = scripts.get(
        "build:integration",
        "",
    )
    ...
    if required_gate not in current_integration:
        old = (
        ...
        if old not in current_integration:
            fail(
                "Unexpected build:integration command; "
                "manual review required."
            )

        scripts["build:integration"] = (
            current_integration.replace(
                old,
                new,
                1,
            )
    ...
    print(
        "FS-1.4.11 cleanup policy applied."
    )
    print()
    print(
        "Created:"
    ...
    print()
    print(
        "AdministrationMockService and "
        "IncidentsMockService remain allowed "
        "as service-level dev/demo/test datasources."
    )
    print()
    print(
        "Validate:"
