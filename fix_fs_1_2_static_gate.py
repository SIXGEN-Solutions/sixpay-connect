...
ROOT = Path.cwd()
TARGET = ROOT / "frontend/scripts/verify-full-stack-conformance.mjs"

NEW_CONTENT = r"""import fs from 'node:fs';
...
if not TARGET.parent.exists():
    raise SystemExit(
        "Run this script from the sixpay-connect repository root."
    )

if TARGET.exists():
    backup_root = (
        ROOT / ".sixpay-recovery"
        / f"fs-1.2-static-gate-{datetime.now():%Y%m%d-%H%M%S}"
    )
