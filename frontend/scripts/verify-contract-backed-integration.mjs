 ...
 */
const environmentIntegration = read('src/environments/environment.integration.ts');

assertContains(environmentIntegration, "mode: 'api'", 'integration environment');

const angularJson = read('angular.json');

assertContains(angularJson, '"integration"', 'Angular integration configuration');
...
const switchedDomains = [
  {
    ...
    apiClient: 'src/app/features/partners/api/partners-api.client.ts',
  ...
  },
  {
    ...
    apiClient: 'src/app/features/payments/api/payments-api.client.ts',
  ...
  },
  {
    ...
    apiClient: 'src/app/features/reporting/api/reporting-api.client.ts',
  ...
  },
  {
    ...
    apiClient: 'src/app/features/accounting/api/accounting-api.client.ts',
  ...
  },
  {
    ...
    apiClient: 'src/app/features/incidents/api/incidents-api.client.ts',
  ...
  },
  {
    ...
    apiClient: 'src/app/features/administration/api/administration-api.client.ts',
  ...
  },
  {
    ...
    apiClient: 'src/app/features/customers/api/customers-api.client.ts',
...
for (const domain of switchedDomains) {
  ...
  assertContains(service, 'BackendModeService', `${domain.name} service`);

  assertContains(service, 'backendMode.usesApi', `${domain.name} service`);

  assertContains(service, domain.apiClientType, `${domain.name} service`);

  assertContains(service, domain.mockType, `${domain.name} service`);
   ...
   */
  if (!service.includes('? this.api') && !service.includes('if (this.backendMode.usesMock)')) {
    fail(`${domain.name} service: no explicit API/mock datasource boundary found`);
  }
}
...
const apiOnlyDomains = [
  {
    ...
    apiClient: 'src/app/features/customers/api/customer-management-api.client.ts',
...
for (const domain of apiOnlyDomains) {
  ...
  assertNotContains(service, 'MockService', `${domain.name} service`);

  assertNotContains(service, 'backendMode.usesMock', `${domain.name} service`);
}
...
for (const expectedDomain of expectedContractBackedDomains) {
  if (!declaredContractBackedDomains.has(expectedDomain)) {
    fail(`FS-1.1 required contract-backed domain is missing: ${expectedDomain}`);
  }
}
...
const explicitlyForbiddenIntegrationMocks = ['AdministrationMockService', 'IncidentsMockService'];
...
for (const root of uiRoots) {
  for (const file of walkFiles(root)) {
    ...
    if (
      [...serviceDirectories].some((serviceDirectory) => rel.startsWith(`${serviceDirectory}/`))
    ) {
      continue;
    }
    ...
    const referencesExplicitlyForbiddenIntegrationMock = explicitlyForbiddenIntegrationMocks.some(
    ...
    if (
      ...
      fail(`${rel}: UI layer must not depend directly on a mock datasource service`);
    }
  }
}
...
for (const serviceFile of readyServiceFiles) {
  ...
  const catchErrorBlocks = source.match(/catchError\s*\([\s\S]{0,500}?\)/g) ?? [];

  for (const block of catchErrorBlocks) {
    if (block.includes('this.mock.') || block.includes('mock.')) {
      fail(`${serviceFile}: silent API-to-mock fallback is forbidden`);
    }
  }
}
...
console.log('FS-1.1 contract-backed integration gate PASSED.');
console.log(
