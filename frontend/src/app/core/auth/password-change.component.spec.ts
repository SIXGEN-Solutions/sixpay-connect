...
describe('PasswordChangeComponent', () => {
  ...
  it('requires matching new-password confirmation', () => {
    const component = fixture.componentInstance as unknown as PasswordChangeComponentTestApi;

    component.form.setValue({
      currentPassword: 'Temporary-password-2026',
      newPassword: 'Permanent-password-2026',
      confirmation: 'Different-password-2026',
    });
  ...
  it('submits current and new password through authentication service', () => {
    ...
    const changePassword = vi
      .spyOn(authentication, 'changeLocalPassword')
      .mockReturnValue(of(undefined));

    component.form.setValue({
      currentPassword: 'Temporary-password-2026',
      newPassword: 'Permanent-password-2027',
      confirmation: 'Permanent-password-2027',
    });
  ...
  it('surfaces backend lifecycle rejection detail to the user', () => {
    ...
    vi.spyOn(authentication, 'changeLocalPassword').mockReturnValue(
      throwError(
        () =>
          new HttpErrorResponse({
            status: 400,
            error: {
              detail: 'Password must not reuse a recent password',
            },
          }),
      ),
    );
    ...
    component.form.setValue({
      currentPassword: 'Temporary-password-2026',
      newPassword: 'Historical-password-2026',
      confirmation: 'Historical-password-2026',
    });

    component.submit();

    expect(component.serverError()).toBe('Password must not reuse a recent password');
  });
});
