...
export const permissionGuard: CanActivateFn = (route) => {
  ...
  const fallbackRoles = (route.data['fallbackRoles'] as readonly SixpayRole[] | undefined) ?? [];

  const authorized =
    permissions.length === 0 ||
    permissions.some((permission) => authentication.hasPermission(permission)) ||
    (authentication.isStandaloneMode && fallbackRoles.some((role) => authentication.hasRole(role)));
