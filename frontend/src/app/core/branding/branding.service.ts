...
import { DEFAULT_INSTITUTION_BRANDING, INSTITUTION_BRANDINGS } from './branding.config';
import {
  InstitutionBranding,
...
export class BrandingService {
  ...
  private readonly activeBrandingState = signal<InstitutionBranding>(this.resolveInitialBranding());
  ...
  selectBranding(id: InstitutionBrandingId): void {
    const branding =
      INSTITUTION_BRANDINGS.find((candidate) => candidate.id === id) ?? INSTITUTION_BRANDINGS[0];
  ...
  private resolveInitialBranding(): InstitutionBranding {
    const storedId = this.storage?.getItem(BRANDING_STORAGE_KEY) as InstitutionBrandingId | null;

    return (
      INSTITUTION_BRANDINGS.find((branding) => branding.id === storedId) ??
      INSTITUTION_BRANDINGS.find((branding) => branding.id === DEFAULT_INSTITUTION_BRANDING) ??
  ...
  private toCssVariables(palette: InstitutionBrandingPalette): Readonly<Record<string, string>> {
    return {
