...
describe('BrandingService', () => {
  ...
  it('applies La Regionale branding tokens', () => {
    ...
    expect(document.documentElement.style.getPropertyValue('--sp-brand-primary')).toBe('#159BD7');
  ...
  it('persists the selected institution for the demo', () => {
    ...
    expect(document.defaultView?.localStorage.getItem('sixpay.ui.institution-branding')).toBe(
      'BICEC',
    );
  });
});
