...
describe(
  'administration API mapper',
  () => {
    it(
      'maps settings without mock-only fields',
      () => {
        expect(
          mapAdministrationSettingsResponse({
    ...
    it(
      ...
      () => {
        ...
        expect(
          mapped.lastSuccessfulAt,
        ).toBeInstanceOf(Date);

        expect(
          mapped.lastCheckedAt,
        ).toBeInstanceOf(Date);
      },
    );

    it(
      'maps overview observedAt',
      () => {
        const mapped =
          mapAdministrationOverviewResponse({
            settings: {
