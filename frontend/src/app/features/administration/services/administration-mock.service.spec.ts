...
describe('AdministrationMockService', () => {
  ...
  it('returns overview', async () => {
    ...
    expect(overview.integrations.length).toBeGreaterThan(0);

    expect(overview.settings.accountingCutoffZone).toBe('Africa/Douala');

    expect(overview.observedAt).toBeInstanceOf(Date);
  });
});
