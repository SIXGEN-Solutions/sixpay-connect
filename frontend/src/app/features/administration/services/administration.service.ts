...
import { AdministrationApiClient } from '../api/administration-api.client';
import {
...
export class AdministrationService {
  ...
  overview(): Observable<AdministrationOverview> {
    return this.backendMode.usesApi
      ? this.api.overview().pipe(map(mapAdministrationOverviewResponse))
      : this.mock.overview();
  }

  settings(): Observable<GeneralSettings> {
    return this.backendMode.usesApi
      ? this.api.settings().pipe(map(mapAdministrationSettingsResponse))
      : this.mock.settings();
  }
  ...
  integrations(): Observable<readonly IntegrationStatus[]> {
    return this.backendMode.usesApi
      ? this.api
          .integrations()
          .pipe(map((responses) => responses.map(mapIntegrationStatusResponse)))
