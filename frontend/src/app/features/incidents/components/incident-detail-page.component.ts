...
@Component({
  ...
  template: `
    <section class="sp-page">
      ...
      @if (loading()) {
      ...
      } @else if (incident(); as currentIncident) {
        <sp-toolbar
        ...
        <sp-card title="Corrélations">
          <dl class="sp-details">
            ...
            <div>
              <dt>Ouvert</dt>
              <dd>
                {{ currentIncident.openedAt | date: 'dd/MM/yyyy HH:mm:ss' }}
              </dd>
            </div>
          </dl>
        </sp-card>
        ...
        <sp-card title="Incident introuvable">
          <p>Aucun incident ne correspond à cet identifiant.</p>
        </sp-card>
      } @else if (loadError()) {
        <sp-card title="Incident indisponible">
          <p>Impossible de charger cet incident.</p>
        </sp-card>
      }
    </section>
  `,
...
export class IncidentDetailPageComponent {
  ...
  constructor() {
    ...
    this.service.get(incidentId).subscribe({
      next: (incident) => {
