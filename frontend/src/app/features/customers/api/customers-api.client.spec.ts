...
describe('CustomersApiClient', () => {
  ...
  it('calls the published ObservedCustomer search endpoint', () => {
    ...
    const request = controller.expectOne((candidate) =>
      candidate.url === '/internal/api/v1/observed-customers',
    );
    ...
    expect(request.request.params.get('lastPaymentStatus')).toBe(
      'TREASURY_INTEGRATED',
    );
    expect(request.request.params.get('lastFailureReasonCode')).toBe(
      'BANK_TIMEOUT',
    );
    expect(request.request.params.get('firstObservedFrom')).toBe(
      '2026-08-01T00:00:00.000Z',
    );
    expect(request.request.params.get('paymentTo')).toBe(
  ...
  it('calls the published linked Payments endpoint with cursor parameters', () => {
    ...
    expect(request.request.params.get('createdFrom')).toBe(
      '2026-08-01T00:00:00.000Z',
    );
