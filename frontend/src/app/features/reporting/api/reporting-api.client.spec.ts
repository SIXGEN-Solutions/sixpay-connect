...
describe('ReportingApiClient', () => {
  ...
  it('calls the published Payment timeline endpoint', () => {
    ...
    const request = controller.expectOne((candidate) =>
      candidate.url ===
      '/internal/api/v1/payments/7fa85f64-5717-4562-b3fc-2c963f66afa1/timeline',
    );

    expect(request.request.method).toBe('GET');
    expect(request.request.params.get('category')).toBe('ACCOUNTING');
    expect(request.request.params.get('occurredFrom')).toBe(
      '2026-08-08T00:00:00.000Z',
    );
    expect(request.request.params.get('occurredTo')).toBe(
  ...
  it('calls the published audit search endpoint with required period', () => {
    ...
    const request = controller.expectOne((candidate) =>
      candidate.url === '/internal/api/v1/payment-audit-records',
    );

    expect(request.request.method).toBe('GET');
    expect(request.request.params.get('paymentReference')).toBe(
      'PAY-2026-0001842',
    );
  ...
  it('calls the published audit detail endpoint', () => {
    client
      .getAudit('22222222-2222-4222-8222-222222222201')
      .subscribe();
  ...
  it('calls the published audit export status endpoint', () => {
    client
      .getExport('33333333-3333-4333-8333-333333333301')
      .subscribe();
