...
const PENDING_PARTNER_ID =
  '10000000-0000-4000-8000-000000000004';

const ACTIVE_PARTNER_ID =
...
test('création et consultation d’un Partner par un administrateur', async ({
  page,
}) => {
  ...
  await page
    ...
    .fill('Alice Admin');
  await page
    .getByLabel('Courriel du contact')
  ...
  await expect(
    page.getByText('PENDING_VALIDATION', { exact: true }),
  ).toBeVisible();
});

test('approbation d’un Partner en attente par un manager', async ({
  page,
}) => {
...
test(
  'configuration d’un seuil, suspension et réactivation par un administrateur',
  async ({ page }) => {
    await useStandaloneRole(page, 'ADMIN');
    ...
    await page
      .getByRole('button', { name: 'Configurer le seuil' })
      .click();
    ...
    await expect(
      page.getByText('SUSPENDED', { exact: true }),
    ).toBeVisible();

    await expect(
      page.getByText('Contrôle de conformité'),
    ).toBeVisible();
...
test('consultation de l’audit Partner par un auditeur', async ({
  page,
}) => {
  ...
  await expect(
    page.getByText('PARTNER_CREATED'),
  ).toBeVisible();

  await expect(
    page.getByText('mock-correlation-created'),
  ).toBeVisible();

  await expect(
    page.getByText('PARTNER_STATUS_OBSERVED'),
  ).toBeVisible();

  await expect(
    page.getByText('mock-correlation-status'),
  ).toBeVisible();
});

test('refuse une route Partner non autorisée', async ({
  page,
}) => {
...
test(
  '@a11y navigation clavier, labels, focus, formulaires et contraste',
  async ({ page }) => {
    await useStandaloneRole(page, 'ADMIN');
    ...
    const legalNameField =
      page.getByLabel('Raison sociale');
    const technicalContactField =
      page.getByLabel('Nom du contact technique');
...
async function useStandaloneRole(
  page: Page,
  role: StandaloneRole,
): Promise<void> {
