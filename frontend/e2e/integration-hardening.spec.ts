...
test.describe('Phase 7.8 integration profile hardening', () => {
  test('uses the API datasource and exposes loading before data', async ({
    page,
  }) => {
    await page.route(
      /\/api\/v1\/partners(?:\?.*)?$/,
    ...
    await expect(
      page.locator('sp-mock-state-panel'),
    ).toHaveCount(0);
  });

  test('surfaces 429 Retry-After and the support correlation id', async ({
    page,
  }) => {
    await page.route(
      /\/api\/v1\/partners(?:\?.*)?$/,
      async (route) => {
        await route.fulfill({
          ...
          body: JSON.stringify({
            ...
            status: 429,
            detail:
              'Le quota temporaire de consultation est atteint.',
  ...
  test('does not expose the mock state panel in integration mode', async ({
    page,
  }) => {
    await page.route(
      /\/api\/v1\/partners(?:\?.*)?$/,
